...
from app.agent.tools import REGISTERED_TOOLS, TOOL_MAP
...
async def run_agent_loop(
    ...
    logger.info(f"Starting agent loop with prompt: {prompt}")
    ...
    for turn in range(max_turns):
        ...
        if response.candidates:
            logger.info(f"Received model response: {response.candidates[0].content}")
            messages.append(response.candidates[0].content)

        # 3. Base Case: If no tools called, stream text and break
        if not response.function_calls:
            logger.info(f"Streaming final response: {response.text}")
